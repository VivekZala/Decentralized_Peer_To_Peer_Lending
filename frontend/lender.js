    ...
    proposals.forEach((proposal) => {
      ...
      if (todayDate.getDate() == date.getDate() + 1) {
        ...
        if(proposal.sendMoney){
          ...
          <td data-label="">
            <a href="#" class="btn btn__secondary" onClick = App.sendMoney(${proposalId}) style="visibility:${vis1}">${string}</a>
          </td>
           <td>
            <a href="./verify.html?a=${proposalId}" class="btn btn__primary">Get Details</a>
<button class="revoke" onClick=App.revokeMortgage(${proposalId}) style="visibility:${vis2}">Revoke Mortgage</button>
          ...
          <td data-label="Due Date">${dueDate}</td>
          <td data-label="">
            <a href="./giveLoan.html?borrower=${proposalId}" class="btn btn__secondary">${string}</a>
          </td>
           <td>
            <a href="./verify.html?a=${proposalId}" class="btn btn__primary">Get Details</a>
            </td>
